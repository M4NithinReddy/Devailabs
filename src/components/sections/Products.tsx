...
const Products = () => {
  ...
  return (
    <section id="products" className="py-32 px-6 bg-surface/30">
      <div className="max-w-screen-2xl mx-auto">
        <div className="w-fit mx-auto text-center mb-20 group">
          <h2 className="font-headline text-4xl font-bold mb-2">Six AI Products. Built for Business.</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {productData.map((product, i) => (
            <motion.div
              ...
              className={`${product.span || ''} p-10 bg-surface rounded-2xl group hover:bg-surface-high transition-all border border-outline-variant/10 relative overflow-hidden`}
            >
              <product.icon className={`w-10 h-10 ${product.color} mb-6`} />
              <h3 className={`font-headline ${product.span ? 'text-3xl' : 'text-2xl'} font-bold mb-4`}>{product.title}</h3>
          ...
          <motion.div
            ...
            onMouseLeave={() => setHoveredProduct(null)}
            className="md:col-span-3 p-12 bg-surface-high relative overflow-hidden rounded-2xl border border-primary/10 group"
          >
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-start gap-12">
              <div className="max-w-xl">
                <span className="text-primary font-label text-[10px] tracking-widest uppercase mb-4 block">New Release</span>
                <h3 className="font-headline text-4xl font-bold mb-6">Automate Pro</h3>
                <p className="text-on-surface-variant mb-8 text-lg leading-relaxed">End-to-end robotic process automation powered by our proprietary Layer 01 intelligence engine. Connect anything, automate everything.</p>
                ...
                >
                  <p className="text-primary text-sm leading-relaxed pb-8 border-b border-outline-variant/10">
                ...
                <button className="primary-gradient text-on-primary px-10 py-4 rounded-md font-bold text-sm">Deploy Automate</button>
              </div>
              <div className="w-full md:w-1/2 flex justify-center">
                <div className="aspect-video w-full max-w-sm glass-card rounded-xl border border-outline-variant/20 flex items-center justify-center">
                  <Settings className="w-24 h-24 text-primary/30 group-hover:scale-110 transition-transform duration-500 animate-spin-slow" />
                </div>
              </div>
            </div>
          </motion.div>
